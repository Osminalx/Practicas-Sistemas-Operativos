...
int buffer_init(buffer_t *, int size){
    buffer->array = mallac(capacity-sizeof(slot_t));
...
int buffer_insert(buffer_t * buffer, slot_t *slot){
    if (buffer->size < buffer->capacity){
        ...
        buffer->he = (buffer->head +1)%buffer->capacity;
        buffer->size++;
        return EXIT_SUCCESS;
    }
    return EXIT_FAILURE;
...
void buffer_detroy(buffer_t*buffer){
    free(buffer->array);
}
...
void buffer_dump(buffer_t*buffer){
    ...
    for (i = 0; i < buffer->size; i++){
        fprintf(stderr, "buffer[%d]: (%d)", j, buffer->array[j].value);
